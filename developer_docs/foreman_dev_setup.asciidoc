...
...
An error occurred while installing pg (`{your_version_number}`), and Bundler cannot continue.
Make sure that `gem install pg -v '`{your_version_number}`' --source 'https://rubygems.org/'` succeeds before bundling.
...

Run the following command:
....
gem install pg -v '`{your_version_number}`' -- --with-pg-config=/usr/pgsql-13/bin/pg_config
....
